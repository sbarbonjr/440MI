...
\title{Class 23: Requirements Engineering for ML Systems}
\subtitle{Session 23 of 36 -- Theory: What to Build and Why}
\author{\coursename}
\date{}
...
\item Specification styles and the requirements document
\item Traceability, change, and requirement management
\end{itemize}
\begin{block}{Note}
The hands-on practice that used to close this topic now has its own session: Class 25, after a real-world bridge with YesAlps in Class 24.
\end{block}
\coursefooter
\end{frame}
...
``The model should be accurate'' is a common ask from stakeholders. Is it a requirement?
\end{block}
\begin{itemize}
\item This forces the distinction between an aspiration and a testable statement.
\item It sets up Class 25, where teams turn this kind of ask into something they could actually verify.
\end{itemize}
\coursefooter
\end{frame}
...
{Class 24 brings in YesAlps, an external partner, to ground requirements and costs in a real engagement before Class 25 puts this theory into practice.}
{Explicit expectations make it possible to choose a realistic development process, which is exactly where the course goes next.}
